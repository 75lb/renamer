...
const { __dirname: moduleDir } = getModulePaths(import.meta.url)
...
export async function loadPlugins (pluginNames) {
  ...
  for (const pluginName of arrayify(pluginNames)) {
    ...
    if (typeof pluginName === 'string') {
      ...
      if (PluginClass === null) {
        PluginClass = await loadModulePathRelativeTo(pluginName, [process.cwd(), path.resolve(moduleDir, './plugin')])
      }
      if (PluginClass === null) {
