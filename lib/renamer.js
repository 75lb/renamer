...
exports.replaceIndexTokenZeroPrefixed = replaceIndexTokenZeroPrefixed
...
function replaceIndexTokenZeroPrefixed (results) {
  results.list = results.list.map(function (result, index) {
    if (result.after) {
      let indexZeroPrefixed = `${index + 1}`
      const lengthOfCurrentIndex = indexZeroPrefixed.length
      const maxZeros = 4
      for (let i = 0; i < maxZeros - lengthOfCurrentIndex; i++) {
        indexZeroPrefixed = `0${indexZeroPrefixed}`
      }
      result.after = result.after.replace(/{{indexZeroPrefixed}}/g, indexZeroPrefixed)
    }
    return result
  })
  return results
