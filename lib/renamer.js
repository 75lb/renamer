...
function replaceIndexToken (results, options) {
  options["num-format"]  = options["num-format"] ? options["num-format"] : "%d"
  results.list = results.list.map(function (result, index) {
    if (result.after) {
      result.after = result.after.replace(/{{index}}/g, sprintf(options["num-format"], index + 1))
    }
    return result
  })
  return results
