...
import DefaultReplace from './plugin/default-replace.mjs'
...
class CliApp {
  ...
  async getPluginArgs (optionDefinitions, argv) {
    /* first parse to fetch the --plugin values */
    let cliArgs = commandLineArgs(optionDefinitions, { partial: true, argv })
    const loadedPlugins = cliArgs.plugin && cliArgs.plugin.length
      ? loadPlugins(cliArgs.plugin)
      : [new DefaultReplace(), new IndexReplace()]
